...
if __name__ == '__main__':
    ...
    if len_argv >= 4:
                ...
                for day in day_list:
                    ...
                    for t in range(int(((86400 / time_length) * 2 - 1))):
                        ...
                        try:
                            ...
                            # band power
                            band_power_temp[counter-1] = band_power(th, fft, band_start=0.1, band_end=1)
                        except:
        ...
        kmeans_result = k_mean_cluster(band_power_db_non_nan, num_cluster)
        ...
        spansum = 0
        # label_color = ['r', 'g', 'b', 'y', 'm']
        label_color = cm.rainbow(np.linspace(0, 1, num_cluster))
        ...
        for i in range(len(place_num)):
            axes1.plot(band_power_db[i][:], linewidth=linewidth, color=color[i])
            ...
            axes1.plot(peaks[0], band_power_db[i][peaks[0]], "o", color=color[i], markersize=2)
        ...
        fig.savefig(output_dir + str.upper(place_name) + "_label_plot.png", dpi=my_dpi)
        ...
        fig.savefig(output_dir + str.upper(place_name) + "_cluster_plot.png", dpi=my_dpi)
        # plt.show()
    else:
