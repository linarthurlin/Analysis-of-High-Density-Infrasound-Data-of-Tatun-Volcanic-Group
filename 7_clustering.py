...
if __name__ == '__main__':
    ...
    if len_argv >= 4:
        ...
        spansum = 0
        bg_color = ['papayawhip', 'mediumpurple', 'crimson', 'steelblue', 'red']
        if (num_cluster > len(bg_color)):
            bg_color = cm.rainbow(np.linspace(0, 1, num_cluster))
        ...
        color = ["red", "limegreen", "royalblue", "orange", "darkslategray", "darkviolet"]
        legend = []
        valleys_save = []
        peaks_save = []
        ...
        fig.savefig(output_dir + str.upper(place_name)+'('+str(band_start)+'~'+str(band_end)+')'+str(place_num)+".png", dpi=my_dpi)
